...
Key endpoints: `create-admin-order.post.ts` (Helcim orders), `helcim.post.ts` (payment processing), `stripe.post.ts` (webhook), `contact.ts` (SendGrid email), `sitemap.xml.ts`, `verify-turnstile.post.ts`, `stock-status.get.ts`.
...
3. **Product data comes straight from GraphQL** — product pages fetch via `GqlGetProduct` inside `useAsyncData`; page-level caching is the ISR/KV route cache. The old `useCachedProduct()`/`cached-products` KV product-JSON layer was removed (July 2026) after it served stale USD prices on test — do not reintroduce a product-JSON KV cache unless a deploy-pipeline writer keeps it in CAD and fresh.
