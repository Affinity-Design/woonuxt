...
Key endpoints: `create-admin-order.post.ts` (Helcim orders), `helcim.post.ts` (payment processing), `stripe.post.ts` (webhook), `contact.ts` (email via Cloudflare Email Service: EMAIL_SENDER service binding → `workers/email-sender`, REST API and SendGrid fallbacks — see `server/utils/emailSender.ts`), `cached-product.ts` (per-slug product KV cache), `sitemap.xml.ts`, `verify-turnstile.post.ts`, `stock-status.get.ts`.
