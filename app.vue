...
// Create a computed property for the robots content
const robotsMetaContent = computed(() => {
  // Defensive check: ensure hostname is a string before using it.
  // Default to 'noindex, nofollow' if hostname is not yet available or not a string during SSR.
  if (typeof hostname !== 'string' || !hostname) {
    return 'noindex, nofollow';
  }
  return hostname === 'proskatersplace.ca' ? 'index, follow' : 'noindex, nofollow';
});
...
useHead({
  ...
  // Define the robots meta tag content conditionally
  // This will set 'index, follow' for 'proskatersplace.ca'
  // and 'noindex, nofollow' for any other hostname (e.g., subdomains, localhost).
  meta: [
    {
      name: 'robots',
      content: robotsMetaContent, // Use the computed property
    },
  ],
});
</script>
