...
- **Status:** ⚠️ **NEEDS KV STORAGE** - API endpoint fails without it
- **Priority:** 🔴 **CRITICAL**
...
- **Status:** ✅ Build-time only (for now)
...
### 6. **blog-redirects.json** (slug → /blog/slug mappings)
...
- **Status:** ⚠️ **ALREADY IN KV** but useSearch.ts imports it locally
- **Priority:** 🟡 Medium (fix local import)
...
- **Status:** ✅ **ALREADY IN KV** as `products-list` key
- **Priority:** 🟢 No action needed
...
- **Status:** ✅ **ALREADY IN KV** as `categories-list` key
- **Priority:** 🟢 No action needed
...
```
npm run build
├── 1. scripts/build-sitemap.js
│   ├── Fetches products, categories, blog posts via GraphQL
│   ├── Generates sitemap-data.json (1,750 routes)
│   ├── Generates product-seo-meta.json (SEO metadata)
│   ├── Generates product-routes.json (for Nuxt config)
│   ├── Generates category-routes.json (for Nuxt config)
│   ├── Generates blog-routes.json (for Nuxt config)
│   └── 🔴 NEW: Upload sitemap-data.json to KV
│
├── 2. scripts/build-products-cache.js
│   ├── Generates products-list (plain text)
│   ├── Generates products-list.json (search index)
│   └── ✅ Already uploads to KV
│
├── 3. scripts/build-categories-cache.js
│   ├── Generates categories-list (plain text)
│   └── ✅ Already uploads to KV
│
└── 4. nuxt build
    └── Reads *-routes.json files for prerendering
```
...
```
├── products-list          (URLs for cache warmer)
├── categories-list        (URLs for cache warmer)
├── cache-warmer-state     (warming progress)
└── sitemap-data           🔴 NEW (complete sitemap)
```

### NUXT_CACHE

```
└── (dynamic route cache - managed by Nitro)
```
...
- ❌ Sitemap endpoint: 22 routes (static fallback)
- ❌ No product pages in sitemap
- ❌ Poor SEO indexing

### After Migration

- ✅ Sitemap endpoint: 1,750 routes
- ✅ All products indexed
- ✅ Google can crawl entire site
...
2. ⏳ Implement Phase 1: Sitemap KV storage
3. ⏳ Test on test branch
4. ⏳ Implement Phase 2: Fix useSearch.ts
5. ⏳ Implement Phase 3: Cleanup orphaned files
