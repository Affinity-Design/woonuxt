...
Hotfix update: discounted orders now skip Helcim's `invoiceRequest` and initialize payment from the final charge `amount` only. WooCommerce remains the source of truth for cart items, coupons, tax, shipping, and order recovery payloads. If this failure appears on a non-discounted order, apply the broader version of this fix by skipping `invoiceRequest` for all HelcimPay.js initializes.

### Phase 1 — Stop the harm + make it visible (SHIPPED)

- **Reclassify** `"Could not complete CC transaction"` as a processing error, NOT a card decline. New honest copy: _"We couldn't process this payment and your card was not charged…"_ + show the "Copy error details" support button for this case. (`HelcimCard.vue`)
