...
> ✅ **IMPLEMENTED (2026-07-17, on `test`) — ALL P0/P1/P2 items:**
> - **P0-1..P0-5 + P1-1 + P1-5** (first pass): no fallback after a captured payment; hard "payment went through — do not pay again" notice with transaction reference + auto-recovery via `/api/recover-helcim-order`; client-minted `checkoutAttemptId` (`composables/useCheckoutAttempt.ts`) enforced at Helcim initialize and in `create-admin-order` idempotency with duplicate-charge stranding; server + client shipping validation; pay affordance hidden while the notice is up.
> - **P1-2**: the attempt-charge marker is now checked/written through **D1 first** (`attempt_charges` table, strongly consistent — no KV propagation race) with KV as fallback/legacy reader (`recordAttemptChargeStrong` / `findRecentAttemptChargeStrong`). Deliberate scope: attempts are marked at charge-*validation*, not modal-open — a modal-open marker can't distinguish "charged, not yet validated" from "closed without paying" and would lock out legitimate abandoned-cart retries; the residual charge→validate window is covered by the order-level attempt idempotency.
> - **P1-3**: `HelcimCard.processPayment()` now ALWAYS re-initializes before opening the pay modal, so the server-side duplicate check runs on every charge attempt (token-reuse bypass closed).
