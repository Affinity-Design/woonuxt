# Blog Keywords Used - Tracking

**Purpose:** Track which keywords from `seo_Keywordlist.csv` have been used in blog posts to prevent keyword cannibalization.

**Last Updated:** 2025-11-13

---

## ✅ Used Keywords

### High Priority Keywords (Already Published)

- [x] **roller skates near me** (5400 sv, 0 kd, $1.09 cpc)

  - Post: `/blog/where-to-buy-roller-skates-near-me-canada`
  - Published: 2025-11-13
  - Status: ✅ Live
  - Performance: Monitoring

- [x] **inline skates canada** (390 sv, 31 kd, $0.45 cpc)

  - Post: `/blog/best-inline-skates-canada-2025`
  - Published: 2025-11-10
  - Status: ✅ Live
  - Performance: Monitoring

- [x] **roller skates for beginners** (estimated 14.8k sv, +1956% YoY)

  - Post: `/blog/complete-beginners-guide-inline-quad-skating`
  - Published: 2025-11-10
  - Status: ✅ Live
  - Performance: Monitoring

- [x] **how to skate backwards** (Featured snippet opportunity)

  - Post: `/blog/how-to-skate-backwards-tutorial`
  - Published: 2025-11-10
  - Status: ✅ Live
  - Performance: Monitoring

- [x] **best inline skates 2025** (High commercial intent)
  - Post: `/blog/best-inline-skates-2025`
  - Published: 2025-11-10
  - Status: ✅ Live
  - Performance: Monitoring

---

## 📋 Available Keywords (Priority Order)

### Ultra High Priority (5000+ sv, Low Competition)

- [ ] **roller skates** (9900 sv, 0 kd, $0.52 cpc)
  - Intent: Informational/Commercial
  - Difficulty: Very Easy
  - Suggested Post: "Complete Roller Skates Guide 2025"
  - Status: Available

### High Priority (1000-5000 sv)

- [ ] **toronto roller skating** (2900 sv, 13 kd, $1.31 cpc)

  - Intent: Local, informational
  - Difficulty: Easy
  - Suggested Post: "Best Roller Skating Spots in Toronto 2025"
  - Status: Available
  - Note: We have `/blog/roller-skating-toronto-guide` - check for overlap

- [ ] **pro skates** (2400 sv, 10 kd, $0.84 cpc)

  - Intent: Commercial/Transactional
  - Difficulty: Easy
  - Suggested Post: "Professional Skates: What the Pros Use"
  - Status: Available

- [ ] **childrens roller skates** (1900 sv, 42 kd, $0.48 cpc)
  - Intent: Commercial/Transactional
  - Difficulty: Medium
  - Suggested Post: "Best Kids Roller Skates 2025 (Safety Guide)"
  - Status: Available

### Medium Priority (500-1000 sv)

- [ ] **skate store toronto** (880 sv, 14 kd, $0.36 cpc)

  - Intent: Local, transactional
  - Difficulty: Easy
  - Suggested Post: "Best Skate Stores in Toronto (Local Shopping Guide)"
  - Status: Available

- [ ] **roller skates for women** (480 sv, 0 kd, $0.42 cpc)

  - Intent: Commercial/Transactional
  - Difficulty: Very Easy
  - Suggested Post: "Best Women's Roller Skates 2025"
  - Status: Available

- [ ] **proskatersplace** (480 sv, 3 kd, $0.00 cpc)

  - Intent: Branded
  - Difficulty: Very Easy
  - Suggested Post: About page optimization
  - Status: Available

- [ ] **pro skate place** (480 sv, 5 kd, $0.00 cpc)
  - Intent: Branded
  - Difficulty: Very Easy
  - Suggested Post: About page optimization
  - Status: Available

### Lower Priority (100-500 sv)

- [ ] **inline skates** (3600 sv, 55 kd, $0.42 cpc)

  - Intent: Commercial/Transactional
  - Difficulty: Medium-Hard
  - Suggested Post: "Inline Skates Ultimate Guide 2025"
  - Status: Available
  - Note: Higher competition, may need multiple posts

- [ ] **online skate shop canada** (390 sv, 6 kd, $0.49 cpc)

  - Intent: Commercial/Transactional
  - Difficulty: Easy
  - Suggested Post: "Best Online Skate Shops in Canada"
  - Status: Available

- [ ] **inline roller skates** (320 sv, 55 kd, $0.30 cpc)

  - Intent: Commercial/Transactional
  - Difficulty: Medium-Hard
  - Suggested Post: Supporting content for main inline skates post
  - Status: Available

- [ ] **mens roller skates** (260 sv, 38 kd, $0.43 cpc)
  - Intent: Commercial/Transactional
  - Difficulty: Medium
  - Suggested Post: "Best Men's Roller Skates 2025"
  - Status: Available

### Niche/Long-tail Keywords (50-100 sv)

- [ ] **quad roller skates** (110 sv, 42 kd, $0.32 cpc)

  - Intent: Commercial
  - Difficulty: Medium
  - Suggested Post: "Quad vs Inline Skates: Complete Comparison"
  - Status: Available

- [ ] **best inline roller skates** (90 sv, 40 kd, $0.65 cpc)

  - Intent: Commercial/Transactional
  - Difficulty: Medium
  - Suggested Post: Supporting content
  - Status: Available

- [ ] **buy roller skates** (90 sv, 55 kd, $0.70 cpc)

  - Intent: Transactional
  - Difficulty: Medium-Hard
  - Suggested Post: "Where to Buy Roller Skates in Canada"
  - Status: Available

- [ ] **inline skate shop** (90 sv, 35 kd, $0.40 cpc)

  - Intent: Commercial/Transactional
  - Difficulty: Medium
  - Suggested Post: About/Shop page optimization
  - Status: Available

- [ ] **pink roller skates** (90 sv, 16 kd, $0.33 cpc)

  - Intent: Commercial
  - Difficulty: Easy
  - Suggested Post: "Best Pink Roller Skates for Kids & Adults"
  - Status: Available

- [ ] **best roller skates** (70 sv, 50 kd, $0.00 cpc)

  - Intent: Commercial/Transactional
  - Difficulty: Medium
  - Suggested Post: General best skates roundup
  - Status: Available

- [ ] **learn to roller skate** (70 sv, 23 kd, $0.94 cpc)
  - Intent: Informational
  - Difficulty: Easy
  - Suggested Post: Tutorial/guide content
  - Status: Available
  - Note: Similar to existing beginner's guide - check overlap

### Very Low Priority (<50 sv or Branded)

- [ ] **buy inline skates** (50 sv, 55 kd, $0.52 cpc)
- [ ] **professional roller skates** (50 sv, 43 kd, $0.00 cpc)
- [ ] **roller skates price** (50 sv, 58 kd, $0.35 cpc)
- [ ] **roller skates for adults** (40 sv, 0 kd, $0.48 cpc)
- [ ] **speed roller skates** (40 sv, 24 kd, $0.00 cpc)
- [ ] **toronto roller skates** (20 sv, 14 kd, $0.00 cpc)

### No Data / Very Low Volume

- [ ] **inline skates pricing** (0 sv, 26 kd, $0.00 cpc)
- [ ] **pro skaters place** (0 sv, 4 kd, $0.00 cpc)
- [ ] **roller skates toronto** (0 sv, 15 kd, $0.00 cpc)
- [ ] **roller skating toronto** (0 sv, 1 kd, $0.00 cpc)

---

## 📊 Keyword Strategy Notes

### Content Gaps to Fill

1. **Local Toronto Content** - Multiple local keywords available

   - toronto roller skating (2900 sv)
   - skate store toronto (880 sv)
   - toronto roller skates (20 sv)

2. **Demographic-Specific**

   - roller skates for women (480 sv)
   - childrens roller skates (1900 sv)
   - mens roller skates (260 sv)

3. **Shopping Intent**

   - roller skates near me (5400 sv) - HIGH PRIORITY
   - buy roller skates (90 sv)
   - online skate shop canada (390 sv)

4. **Educational Content**
   - learn to roller skate (70 sv)
   - How to maintain skates
   - How to choose skates

### Keyword Cannibalization Warnings

⚠️ **Be careful with these overlapping keywords:**

- "inline skates" vs "inline roller skates" vs "roller skates"
- "toronto roller skating" vs "roller skating toronto" vs "toronto roller skates"
- "best roller skates" vs "best inline skates" vs "professional roller skates"

**Solution:** Use as secondary keywords in different posts, each with unique primary focus.

---

## 🎯 Next Blog Post Recommendations

### Immediate Priorities (Write Next)

1. **"Where to Buy Roller Skates Near You in Canada 2025"**

   - Primary: roller skates near me (5400 sv, $1.09 cpc)
   - Secondary: buy roller skates, roller skates price
   - Type: Local + Commercial
   - Internal links: Store locations, product categories

2. **"Best Roller Skating Spots in Toronto 2025"**

   - Primary: toronto roller skating (2900 sv, $1.31 cpc)
   - Secondary: roller skating toronto, toronto roller skates
   - Type: Local guide
   - Internal links: Existing toronto guide, products

3. **"Best Kids Roller Skates 2025 (Complete Safety Guide)"**
   - Primary: childrens roller skates (1900 sv, $0.48 cpc)
   - Secondary: roller skates for adults (comparison)
   - Type: Buying guide + safety
   - Internal links: Protection gear, kids products

### Medium-Term (2-4 weeks)

4. **"Best Women's Roller Skates 2025"**
5. **"Professional Skates: What the Pros Use"**
6. **"Complete Roller Skates Guide 2025"** (broad keyword)

---

## 📝 Update Instructions

**When using a keyword:**

1. Move keyword from "Available" to "Used" section
2. Add post URL, publish date, and status
3. Update "Last Updated" date at top
4. Add performance notes after 30 days

**Format for new entries:**

```markdown
- [x] **keyword phrase** (sv, kd, $cpc)
  - Post: `/blog/post-slug`
  - Published: YYYY-MM-DD
  - Status: ✅ Live / 🚧 Draft / 📝 Planning
  - Performance: [Rankings/Traffic notes after 30 days]
```

---

**Maintained By:** SEO Team  
**Review Frequency:** Weekly  
**Last Reviewed:** 2025-11-13
