/**
 * PSP HPOS Sequential Order Number Bridge
 *
 * Ensures HPOS/API-created WooNuxt orders receive SkyVerge Sequential Order Numbers.
 * Paste into Code Snippets as a PHP snippet, scope: "Run everywhere", then activate.
 */

if (
	defined( 'PSP_HPOS_ORDER_NUMBER_BRIDGE_LOADED' )
	|| function_exists( 'psp_assign_missing_sequential_order_number_for_hpos_order' )
	|| function_exists( 'psp_hpos_order_tables_are_active' )
	|| function_exists( 'psp_order_already_has_sequential_order_number' )
	|| function_exists( 'psp_backfill_missing_sequential_order_numbers_command' )
) {
	return;
}

define( 'PSP_HPOS_ORDER_NUMBER_BRIDGE_LOADED', true );
