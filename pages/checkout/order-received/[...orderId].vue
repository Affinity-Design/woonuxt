...
const getPaymentMethodDisplay = computed(() => {
  ...
  const isHelcimPayment =
    ...
    order.value.metaData?.some(
      (meta: any) => meta.key === "_payment_method" && meta.value === "helcim"
    );
...
onMounted(async () => {
  ...
  if (
    initialOrderFetchSucceeded.value &&
    isCheckoutPage.value &&
    fetchDelay.value &&
    orderIsNotCompleted.value
  ) {
    setTimeout(() => {
      getOrder();
    }, delayLength);
...
async function getOrder() {
  ...
  try {
    ...
    if (data?.order) {
      ...
      let errorDetail =
        "Order not found or GraphQL query returned no order data.";
...
<template>
  ...
  >
    ...
    <template v-else>
      <div v-if="order" class="w-full">
        ...
        <template v-if="isCheckoutPage">
          <div class="my-4 text-center">
            ...
            <p v-if="query.key" class="font-bold text-gray-800">
              Reference: {{ query.key }}
            </p>
            <div class="mt-2 text-sm text-gray-600">
              <p>We sent you an email confirmation.</p>
              <p>
                We will email you again when your order is shipped or the status
                has changed.
              </p>
            ...
            <div class="mt-6 flex justify-center">
              <NuxtLink
                to="/products"
                class="bg-primary hover:bg-primary-dark text-white px-6 py-3 rounded-lg"
                >Return to Shop</NuxtLink
              >
            </div>
          </div>
        </template>
      ...
      <!-- Error message -->
      <div
        v-if="errorMessage && !order"
        class="flex flex-col items-center justify-center flex-1"
      >
        <Icon
          name="ion:alert-circle-outline"
          size="64"
