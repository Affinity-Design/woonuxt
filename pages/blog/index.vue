...
// Category filtering
const categories = computed(() => Array.from(new Set(posts.value?.map((p) => p.category).filter(Boolean))).sort());
...
const filteredPosts = computed(() => {
  ...
  return activeCategory.value === 'All' ? posts.value : posts.value.filter((p) => p.category === activeCategory.value);
});
    ...
    <div class="mx-auto" style="max-width: 1320px; padding: clamp(24px, 5vw, 64px)">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div class="col-span-full flex flex-wrap gap-2 mb-4">
          <button
            ...
            :class="[
              ...
              activeCategory === category ? 'bg-white text-black' : 'bg-gray-800 text-white border border-gray-600',
            ]"
        ...
        <NuxtLink
          ...
          <div class="p-6">
            ...
            <div class="flex items-center gap-2 mb-3">
              <!-- Category Chip -->
              <span v-if="post.category" class="inline-block px-3 py-1 text-xs font-medium text-gray-600 bg-gray-100 rounded-full">
                {{ post.category }}
              </span>
            ...
            <!-- Title -->
            <h2 class="text-xl font-semibold text-black group-hover:underline line-clamp-2">
              {{ post.title }}
            </h2>

            <!-- Description -->
            <p v-if="post.description" class="mt-2 text-gray-600 text-sm line-clamp-2">
              {{ post.description }}
            </p>
          </div>
        </NuxtLink>
        ...
        <div v-if="filteredPosts.length === 0" class="col-span-full text-center py-16">
          <Icon name="ion:document-text-outline" class="text-gray-300 text-6xl mb-4" />
