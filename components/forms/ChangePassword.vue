...
const updatePassword = async () => {
  ...
  try {
    ...
    if (updateCustomer) {
      ...
      const loginResult = await loginUser({username: viewer.value.username, password: password.value.new});
      if (!loginResult.success) {
        errorMessage.value = loginResult.error || getSafePublicErrorMessage(null);
    ...
    console.error('Password update failed:', getSafeErrorLogDetails(error));
    errorMessage.value = getSafePublicErrorMessage(error, 'We could not update your password. Please try again.');
    button.value = {text: t('messages.account.failed'), color: 'bg-red-500'};
  } finally {
    loading.value = false;
  }
...
<template>
  <form class="bg-white rounded-lg shadow mt-4" @submit.prevent="updatePassword">
    <div class="grid p-8 gap-6 md:grid-cols-2">
      ...
      <input type="text" v-model="viewer.username" name="username" autocomplete="username" class="hidden" />
      ...
      <Transition name="scale-y" mode="out-in">
        <div v-if="errorMessage" class="w-full text-sm text-red-500">{{ errorMessage }}</div>
      </Transition>
    </div>
