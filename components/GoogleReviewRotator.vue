...
const reviews = [
  ...
  },
  {
    ...
    rating: 5,
    date: '1 month ago',
    text: "Accurate, timely order processing. Super helpful staff. I couldn't be happier with the overall experience with ProSkaters Place. Thanks @ Marina and team.",
  },
  {
    name: 'christ13c',
    rating: 5,
...
const maxIndex = computed(() => {
  ...
  return reviews.length > itemsPerView.value ? reviews.length - itemsPerView.value : 0;
});
  ...
  <section class="container my-16">
    ...
    <div v-if="isMounted" class="relative max-w-6xl mx-auto overflow-hidden">
      <div
        ...
        <div v-for="(review, idx) in reviews" :key="idx" class="flex-shrink-0 px-3" :style="{width: 100 / reviews.length + '%'}">
          <div class="bg-white rounded-lg p-6 shadow h-full flex flex-col min-h-[280px]">
            ...
            <p class="text-center text-gray-700 text-sm flex-grow">"{{ review.text }}"</p>
          </div>
        </div>
      </div>
    ...
    <div v-else class="relative max-w-6xl mx-auto text-center py-10 min-h-[320px] flex items-center justify-center">
